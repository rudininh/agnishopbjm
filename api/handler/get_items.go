...
func GetItemsHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Ambil token Shopee
	var partnerID, shopID int64
	var accessToken string
	err := db.QueryRow("SELECT partner_id, shop_id, access_token FROM shopee_tokens LIMIT 1").
		Scan(&partnerID, &shopID, &accessToken)
	if err != nil {
		http.Error(w, fmt.Sprintf("Gagal ambil token Shopee: %v", err), http.StatusInternalServerError)
		return
	}

	// Timestamp dan tanda tangan
	timestamp := time.Now().Unix()
	partnerKey := config.GetShopeePartnerKey()
	baseURL := "https://partner.shopeemobile.com/api/v2/product/get_item_list"

	// Buat signature
	signature := config.GenerateShopeeSign(partnerKey, partnerID, shopID, timestamp, "product/get_item_list")

	url := fmt.Sprintf("%s?partner_id=%d&shop_id=%d&timestamp=%d&sign=%s&access_token=%s",
		baseURL, partnerID, shopID, timestamp, signature, accessToken)

	fmt.Println("📡 [DEBUG] URL Shopee Get Item List:", url)

	resp, err := http.Get(url)
	if err != nil {
		http.Error(w, fmt.Sprintf("Gagal request ke Shopee: %v", err), http.StatusInternalServerError)
		return
	}
	defer resp.Body.Close()
	...
	if err := json.Unmarshal(body, &result); err != nil {
		http.Error(w, fmt.Sprintf("Gagal decode JSON Shopee: %v", err), http.StatusInternalServerError)
		return
	}

	response, ok := result["response"].(map[string]interface{})
	if !ok {
		http.Error(w, "Format respons Shopee tidak sesuai", http.StatusInternalServerError)
		return
	}

	itemList, ok := response["item"].([]interface{})
	if !ok {
		http.Error(w, "Item list tidak ditemukan", http.StatusInternalServerError)
		return
	}
	...
	for _, item := range itemList {
		...
		itemID := int64(itemMap["item_id"].(float64))
		name := itemMap["item_name"].(string)

		// Ambil detail harga & stok per produk
		detailURL := fmt.Sprintf("https://partner.shopeemobile.com/api/v2/product/get_item_base_info?partner_id=%d&shop_id=%d&timestamp=%d&sign=%s&access_token=%s&item_id_list=[%d]",
			partnerID, shopID, timestamp, signature, accessToken, itemID)

		fmt.Println("🔍 [DEBUG] URL Get Item Detail:", detailURL)

		detailResp, err := http.Get(detailURL)
		if err != nil {
			fmt.Println("❌ Gagal ambil detail item:", err)
			continue
		}
		...
		if responseDetail, ok := detailResult["response"].(map[string]interface{}); ok {
			if itemsDetail, ok := responseDetail["item_list"].([]interface{}); ok && len(itemsDetail) > 0 {
				if firstItem, ok := itemsDetail[0].(map[string]interface{}); ok {
					// Ambil harga
					if priceInfo, ok := firstItem["price_info"].([]interface{}); ok && len(priceInfo) > 0 {
						if firstPrice, ok := priceInfo[0].(map[string]interface{}); ok {
							if currentPrice, ok := firstPrice["current_price"].(float64); ok {
								hargaStr = strconv.FormatFloat(currentPrice/100000.0, 'f', 0, 64)
							}
						}
					}

					// Ambil stok
					if stockInfo, ok := firstItem["stock_info_v2"].(map[string]interface{}); ok {
						if summary, ok := stockInfo["summary_info"].(map[string]interface{}); ok {
							if totalStock, ok := summary["total_available_stock"].(float64); ok {
								stockInt = int(totalStock)
							}
						}
					}
				}
